        ...
        <tbody>
          {tasks.map((t) => (
            <tr key={t.task_id} className="border-b hover:bg-neutral-50">
              ...
              <td className="px-2 py-2 text-right">{t.criteria_failed ?? "—"}</td>
              <td className="px-2 py-2 text-right">
                {Number(t.reviewed_count) > 0 && t.criteria_failed != null ? `${t.reviewed_count}/${t.criteria_failed}` : "—"}
              </td>
              <td className="overflow-hidden px-2 py-2 text-left whitespace-nowrap">
                {t.task_stale && (
                  <span title={TASK_STALE_TOOLTIP} className="mr-1 rounded bg-amber-100 px-1.5 py-0.5 text-xs text-amber-800">
                    task stale
                  </span>
                )}
                {t.rubric_stale && (
                  <span title={RUBRIC_STALE_TOOLTIP} className="rounded bg-amber-100 px-1.5 py-0.5 text-xs text-amber-800">
                    rubric stale
                  </span>
                )}
              </td>
